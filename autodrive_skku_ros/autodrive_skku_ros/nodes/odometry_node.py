...
def fuse(vo, cmd, odo_cfg):
    """VO(estimate_visual_motion 반환값)와 커맨드-적분((dx,dy,dtheta) 튜플)의
    상보 필터(complementary filter) 융합 — 풀 EKF가 아니라 인라이어 수 기반
    가중치로 섞는 가벼운 방식. vo["ok"]가 False면 confidence=0(커맨드-적분만
    사용). 반환: (dx_m, dy_m, dtheta_rad, confidence)."""
    cmd_dx, cmd_dy, cmd_dtheta = cmd
    if not vo["ok"]:
        return cmd_dx, cmd_dy, cmd_dtheta, 0.0
    min_inliers = odo_cfg["vo_min_inliers"]
    weight_max = odo_cfg["fusion_vo_weight_max"]
    confidence = min(1.0, vo["n_inliers"] / float(min_inliers)) * weight_max
    dx = confidence * vo["dx_m"] + (1.0 - confidence) * cmd_dx
    dy = confidence * vo["dy_m"] + (1.0 - confidence) * cmd_dy
    dtheta = confidence * vo["dtheta_rad"] + (1.0 - confidence) * cmd_dtheta
    ...
    class OdometryNode(Node):
        ...
        def _on_front(self, msg):
            ...
            if dt is not None and dt > 0:
                ...
                dx, dy, dtheta, confidence = fuse(vo, cmd, config.ODOMETRY)
                self.x, self.y, self.theta = compose_pose(self.x, self.y, self.theta, dx, dy, dtheta)
...
def selftest():
    ...
    선택, 상보 필터 융합, pose 합성, 지면 투영 호모그래피, VO)만 검증한다."""
    checks = []
    ...
    inert_cfg = dict(pwm_to_mps=None, deg_per_pulse=None, min_pot_span_counts=8,
                      vo_min_features=15, vo_min_inliers=10, fusion_vo_weight_max=0.8,
    ...
    # ---- fuse ----
    cmd = (1.0, 0.5, 0.2)
    dx, dy, dtheta, conf = fuse(_vo_empty(ok=False), cmd, cal_cfg)
    check("VO ok=False면 confidence=0, 커맨드-적분 값 그대로",
          conf == 0.0 and (dx, dy, dtheta) == cmd)
    vo_full = dict(dx_m=2.0, dy_m=0.0, dtheta_rad=0.0,
                   n_tracked=50, n_inliers=cal_cfg["vo_min_inliers"] * 2, ok=True)
    dx2, dy2, dtheta2, conf2 = fuse(vo_full, cmd, cal_cfg)
    check("인라이어 충분하면 confidence가 fusion_vo_weight_max에서 포화",
          abs(conf2 - cal_cfg["fusion_vo_weight_max"]) < 1e-12)
    check("포화 confidence로 블렌딩된 dx가 VO/커맨드 사이 값",
          min(vo_full["dx_m"], cmd[0]) <= dx2 <= max(vo_full["dx_m"], cmd[0]))
