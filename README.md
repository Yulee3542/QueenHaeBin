...
아래는 실제로 처음 받아서 실차를 몰기까지 진행하는 순서대로 정리돼 있다: **최초 설정 → (WSL2면) USB 연결 → 코드 업데이트 → 펌웨어 업로드 → 첫 주행 체크리스트 → 이후 매번 사용법**. 참고 자료(미션 상세, 아키텍처, 디버깅, 문제 해결)는 맨 아래.
...
**WSL2에서 개발 중이면 `check_env.py`가 카메라/시리얼을 제대로 잡으려면 먼저 [2. WSL2 USB 장치 연결](#2-wsl2-usb-장치-연결-wsl2-개발-환경만) 절차부터 해야 한다** (`setup.sh` 자체는 하드웨어 없이도 끝까지 됨). 대회 실차(Ubuntu 직결)라면 이 단계는 건너뛰고 바로 3번으로.

---

## 2. WSL2 USB 장치 연결 (WSL2 개발 환경만)
...
**매 사용 세션 시작할 때(아래 [7. 사용법](#7-사용법-설정이-끝난-뒤-매번-이렇게)으로 가기 전)도 이 attach부터** — 포트를 바꾸지 않았어도 WSL을 재시작했거나 장치를 재연결했으면 다시 필요하다.
...
- 아두이노가 시리얼 연결 시 리셋되며 USB 장치로 잠깐 사라졌다 나타날 수 있음(이때 attach가 끊기므로 `--auto-attach` 권장).

---

## 3. 코드 업데이트 반영 (2회차 이후)

최초 설정 이후 최신 코드만 반영하려면 `setup.sh` 대신 `update.sh`:
...
`git pull --ff-only`가 실패하면(로컬에 커밋 안 된 실차 튜닝 등) 스크립트가 멈춘다 — 자동 머지·강제 덮어쓰기 안 하므로 `git status`로 직접 확인 후 처리할 것. 파이썬 파일만 바뀐 경우 `colcon build --symlink-install`(두 스크립트가 이미 이 옵션으로 빌드) 덕분에 재빌드 없이 바로 반영된다 — `setup.py`/`package.xml`(새 노드 실행파일 추가 등)이 바뀔 때만 재빌드 필요.

---

## 4. 펌웨어 업로드 (아두이노 메가)

`arduino/car_controller/car_controller.ino`를 아두이노 메가 2560에 업로드(외부 라이브러리 불필요). `.ino`는 파이썬/ROS와 별개로 컴파일·업로드되는 펌웨어다 — 한 번 올리면 보드가 독립적으로 계속 실행하고, 파이썬 쪽은 시리얼로 통신만 한다.

업로드 방법 두 가지:
- **WSL2 안에서 GUI 없이**: `./tools/upload_firmware.sh` (최초 실행 시 `arduino-cli` + `arduino:avr` 코어 자동 설치, 포트 생략 시 자동 감지)
- **Windows Arduino IDE**: usbipd로 WSL에 attach된 상태면 `usbipd detach --busid <ID>`로 먼저 풀고, `\\wsl.localhost\<배포판>\home\<user>\...\arduino\car_controller\car_controller.ino` 경로로 열어 업로드(보드: "Arduino Mega or Mega 2560"). 업로드 후 다시 `usbipd attach --wsl --busid <ID>`.
...
| 차량→PC | `P <adc>` | 조향 POT 원시값(A2, 0~1023), 50ms마다 — 항상 보냄(POT 미장착이면 플로팅값이라 의미 없음) |

안전 장치:
- **워치독**: `V` 명령 수신 후 500ms 이상 시리얼이 끊기면 자동 정지 (파이썬 쪽은 200ms마다 keepalive 전송)
- `V`를 한 번도 받지 못하면 G/2/S 수동 명령으로 동작 (구버전 호환)

### 조향 POT 자동 캘리브레이션 (선택 하드웨어)

조향 링키지에 가변저항(POT)을 달고 와이퍼 핀을 아두이노 메가의 **A2**에 연결하면(전용 보드 아니고 이 메가에 직결 — GND/5V도 같이 배선), `arduino_node`가 뜰 때마다 `calibrate_steering:=true`(기본값)로 자동 캘리브레이션한다:

1. `steer_pulse("L")` 반복 → POT ADC가 더 이상 안 바뀌면(기계적 풀락) 정지
2. 반대쪽도 `steer_pulse("R")`로 동일하게 진행
3. 두 풀락 값의 중간으로 조향을 되돌림
4. 이후 `/car/steering_pot`(raw ADC), `/car/steering_angle`(±`STEERING_LIMIT_DEG` deg)를 계속 발행

POT이 없으면(펄스를 줘도 ADC가 안 바뀌면) 자동으로 조용히 스킵되고 기존 펄스 방식 그대로 동작 — 항상 켜둬도 안전하다. 단, **캘리브레이션 중 바퀴가 실제로 좌우로 움직이므로** 바퀴를 띄우거나 장애물 없는 곳에서 기동할 것(정 안 되면 `calibrate_steering:=false`).

---

## 5. 실차 첫 주행 체크리스트

앞의 1~4단계를 마쳤다면, 정식으로 매번 사용하기 전에 딱 한 번 이 순서로 검증한다:

1. `car_controller.ino` 업로드 확인 (위 [4. 펌웨어 업로드](#4-펌웨어-업로드-아두이노-메가) 절 참고)
2. `python3 tools/check_env.py` — 장치/ROS 패키지 인식 확인
3. 바퀴를 띄운 상태에서 `python3 tools/hw_test.py` — 전진/조향 모듈 개별 확인 (막히면 [디버깅 사다리](#디버깅-사다리-ros-없이-하드웨어만-직접-확인) 참고)
...
여기까지 통과하면 이후로는 아래 [7. 사용법](#7-사용법-설정이-끝난-뒤-매번-이렇게)만 반복하면 된다.
...
| `test` 수동 테스트 | 자동주행 없음 — 키보드 텔레옵 (`ros2 run`으로 직접 실행 필요) | 동작 |

각 미션 파일(`autodrive_skku_ros/autodrive_skku_ros/missions/*.py`) 상단 docstring에 세부 목표와 채워야 할 `TODO` 메서드가 정리돼 있다 — **담당 팀은 자기 미션 파일의 TODO 메서드만 채우면 된다** (`Mission.step(sensors, car)` 인터페이스는 고정).

새 미션 추가: `Mission`(`missions/base.py`)을 상속한 클래스를 만들고 `missions/__init__.py`의 `MISSIONS`에 등록하면 메뉴에 자동으로 나타난다.

---

## 7. 사용법 (설정이 끝난 뒤 매번 이렇게)

WSL2 개발 환경이면 먼저 [2. WSL2 USB 장치 연결](#2-wsl2-usb-장치-연결-wsl2-개발-환경만)의 `usbipd attach`부터.
...
- **`mission:=`는 필수다** (`run_mission:=true`가 기본값일 때). `ros2 launch`는 자식 프로세스의 stdin을 연결하지 않는 ROS 2 launch 자체의 알려진 제약([ros2/launch#735](https://github.com/ros2/launch/issues/735)) 때문에 대화형 메뉴가 여기서는 안 뜬다 — 생략하면 에러 메시지 내고 바로 종료. 메뉴가 필요하면 다른 노드는 launch로 띄운 채 `ros2 run autodrive_skku_ros mission_node`만 별도 터미널에서 직접 실행할 것(이 경우 stdin 정상 동작).
- **종료는 `Ctrl+C` 한 번** — SIGINT/SIGTERM 둘 다 모터에 정지 신호를 보낸 뒤 종료된다(안전 종료 처리됨).
- 이상하면 [문제 해결](#문제-해결) 표부터 볼 것.

### 미션 선택

| 미션 | 용도 |
|------|------|
| `road` | 기본 도로 주행 — 차선 인식 + 장애물 회피 차선 변경 |
| `traffic` | 신호등/정지선 구간 |
| `t_parking` | T자 주차 구간 |
| `test` | 자동주행 없음, 키보드로 모터/조향 수동 조종 (정비·점검용, 아래 참고) |

### 자주 쓰는 launch 인자

| 인자 | 설명 |
|------|------|
| `mission:={road,traffic,t_parking}` | 위 표 참고 (`run_mission:=true`면 필수) |
| `run_mission:=false` | `mission_node` 없이 센서/액추에이터만 기동 (아래 "수동 모터 테스트") |
| `show:=true` | 카메라 창 표시 (`q`로 종료, 디스플레이 있는 환경 한정) |
| `calibrate_steering:=false` | 기본 `true` — 조향 POT 좌/우 풀락 자동 탐색(바퀴가 몇 초간 움직임, POT 미장착이면 자동 스킵). 바퀴를 못 움직이는 상황에서만 끌 것 |
| `arduino_port:=/dev/ttyACM0` / `lidar_port:=/dev/ttyUSB0` | 자동 감지 실패 시 직접 지정 |
| `front_camera:=0` / `rear_camera:=2`(`-1`=미사용) | 카메라 인덱스 |
| `foxglove_port:=8765` | Foxglove WebSocket 포트 |

### 수동 모터 테스트 (미션 없이)

가장 간단한 방법은 `mission:=test`다 — **`test` 미션 자체가 키보드 텔레옵을 겸한다**:

```bash
ros2 launch autodrive_skku_ros bringup.launch.py run_mission:=false
# 별도 터미널에서 (stdin 필요해 ros2 run으로 직접 실행):
ros2 run autodrive_skku_ros mission_node --ros-args -p mission:=test
```
Enter 없이 키 하나로 즉시 반영: `g`=주행 허용, `w`/`x`=속도 ±20, `space`=속도 0, `a`/`d`=좌/우 조향 펄스, `f`=조향 중립, `s`=정지, `h`=도움말.

별도 프로세스로 조종하려면 `ros2 run autodrive_skku_ros teleop_node`(어떤 미션이 떠 있든 무관하게 `/car/cmd/*`에 직접 발행), 또는 한 번씩 명령만 보내려면:
...
```
Foxglove 앱의 "Publish" 패널로도 같은 토픽에 발행 가능. 문자열 payload(`'F'`/`'L'`/`'R'`)는 실제로 아두이노에 전송되는 시리얼 바이트와 같다.

### Foxglove로 모니터링

1. [Foxglove 앱](https://foxglove.dev/download) 실행 → "Open connection" → "Foxglove WebSocket" → `ws://<차량 IP 또는 localhost>:8765`
2. 아래 토픽을 패널에 추가 (전체 목록은 [Foxglove 토픽](#foxglove-토픽) 참고)

WSL2 개발 중이면 Windows 쪽 Foxglove는 WSL 내부 IP(`ip addr show eth0`)로 접속.
...
- 전방 **C920 한 대**의 프레임을 상/하로 분할 — `sensors["top"]`=상단(신호등), `sensors["bottom"]`=하단(차선).
- 전방 카메라는 **portrait(세로) 마운트**가 기준(수직 화각 ≈78°). C920은 하드웨어 회전이 없어 `config.FRONT_CAMERA_ROTATE`(`"CW"`/`"CCW"`/`"180"`)로 캡처 후 소프트웨어 보정 — 실차에서 좌우가 반전되면 이 값부터 확인.
- 후방 카메라는 `config.py`의 `REAR_CAMERA` 또는 `rear_camera:=` 인자로 지정하면 `sensors["rear"]`로 들어온다(T주차용, 회전 보정 없음).
- 차선 인식은 `vendor/Function_Library.py`의 `edge_detection`을 그대로 사용, 파라미터는 `missions/lane_follow.py`의 `LANE_EDGE`. `road`/`traffic` 미션이 같은 파일의 `follow_lane()`을 공유 호출.
...
각 `nodes/*.py`는 `python3 -m autodrive_skku_ros.nodes.<파일명> --selftest`로 ROS 없이 바로 검증할 수 있다(예: `arduino_node --selftest`). 하드웨어 공용 설정은 `config.py`, 미션별 튜닝값은 그 값을 실제로 쓰는 `missions/*.py` 파일 상단에 있다 — 둘 다 ROS 파라미터화하지 않고 파이썬 모듈 상수로 뒀다(대회 전 회귀 위험 최소화).
...
```
이것도 안 움직이면 핀/배선/모터 드라이버 전원(아두이노 USB 전원과 별개로 구동 전원이 들어가는지) 문제로 좁혀진다. 확인 후 반드시 `./tools/upload_firmware.sh`로 `car_controller.ino`를 다시 올릴 것.
...
| 증상 | 해결 |
|------|------|
| `ros2 launch`에서 미션 메뉴가 안 뜸/입력이 안 먹힘 | `mission:=road`처럼 launch 인자로 미리 지정할 것 — [7. 사용법](#7-사용법-설정이-끝난-뒤-매번-이렇게) 참고 |
| 기동할 때마다 바퀴가 몇 초간 좌우로 저절로 움직임 | 조향 POT 자동 캘리브레이션(정상 동작) — 원치 않으면 `calibrate_steering:=false` |
| 조향이 계속 한쪽으로 감 | 펄스 방식이라 자동 복원 안 됨 — 반대 방향 펄스로 복귀. POT이 달려 있으면 기동할 때마다 자동으로 중앙 복귀됨 |
| 차가 안 움직임 | 미션이 `car.go()`를 호출했는지, 펌웨어 업로드 여부 확인. `teleop_node`/`test` 미션이면 **`w`/`x` 전에 반드시 `g`부터** — 워치독 게이트(`canGo`)가 안 열려 있으면 속도는 받아도 구동 0으로 처리됨(`s`를 누르면 게이트가 다시 닫힘) |
| `/dev/ttyUSB0` permission denied | `./setup.sh`가 dialout 그룹에 추가함 — **재로그인** 필요 |
| 카메라 열기 실패 | 다른 프로그램이 점유 중인지 확인, WSL2면 [usbipd attach](#2-wsl2-usb-장치-연결-wsl2-개발-환경만) |
| 아두이노/라이다 포트 뒤바뀜 | `arduino_port:=`/`lidar_port:=` 인자로 직접 지정 |
| `/scan`의 좌우/전후가 기대와 다름 | `rplidar_ros` 각도 규약이 기존 파이썬 `rplidar` 라이브러리와 다를 수 있음 — `config.LIDAR_MOUNT`(`yaw_offset_deg`/`invert`)를 실차에서 재보정 |
| `colcon build`가 무관한 다른 패키지 때문에 실패/중단됨 | `colcon build --packages-up-to autodrive_skku_ros`로 대상 좁힐 것(`setup.sh`/`update.sh`는 이미 이렇게 함). `.venv` 등 가상환경 활성화 상태면 `deactivate` 후 시도(ROS 2 전환 후엔 `.venv` 자체가 불필요) |
