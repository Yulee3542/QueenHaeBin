...
│   │   │                               #   TRAFFIC_PIXEL_RATIO/TRAFFIC_LIGHT=traffic.py, T_PARKING=t_parking.py)
│   │   │                               #   occupancy.py = T주차 점유 격자 (--selftest 가능)
│   │   │                               #   traffic_yolo.py = 신호등 YOLO 검출기(1순위, --selftest 가능,
│   │   │                               #   ultralytics 미설치 시 자동으로 None → traffic.py가 HSV만 사용)
│   │   ├── models/                     # traffic_light.pt — 팀 공용 YOLO 신호등 가중치 (출처는 models/README.md)
...
├── requirements.txt                    # rplidar-roboticia(vendor 강제 임포트용) + ultralytics(선택, YOLO 신호등)
...
ros2 param set /mission_node traffic_light.conf 0.5           # YOLO confidence 임계 (기본 0.35)
ros2 param set /mission_node traffic_light.use_yolo false     # YOLO 끄고 HSV만 사용
